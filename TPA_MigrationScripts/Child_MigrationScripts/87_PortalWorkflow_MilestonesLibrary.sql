...
/*PortalWorkflow_MilestonesLibrary Migration*/
BEGIN TRANSACTION;  
  
BEGIN TRY  
--Get max ID value in table
DECLARE @MaxIDExist INT = (SELECT MAX(MilestoneId) FROM [dbo].[PortalWorkflow_MilestonesLibrary]);--353

--Insert @MaxIDExist into respective mapping table
	SET IDENTITY_INSERT dbo.[PortalWorkflow_MilestonesLibrary_ID_Mapping] ON; 
	INSERT INTO [dbo].[PortalWorkflow_MilestonesLibrary_ID_Mapping] (MapId, Old_ID, New_ID)
	VALUES(-1, @MaxIDExist, -1)
	SET IDENTITY_INSERT dbo.[PortalWorkflow_MilestonesLibrary_ID_Mapping] OFF; 

--UPDATE TriggerId column
UPDATE [dbo].[PortalWorkflow_MilestonesLibrary_MP]
SET TriggerId = um.New_ID
FROM [dbo].[PortalWorkflow_MilestonesLibrary_MP] bc
...
SELECT @LoopCounter = MIN(LoopId),
		@MaxId = MAX(LoopId) 
FROM [dbo].[PortalWorkflow_MilestonesLibrary_MP]

IF ((SELECT COUNT(*) FROM [dbo].[PortalWorkflow_MilestonesLibrary_MP]) > 0)
BEGIN
WHILE ( @LoopCounter IS NOT NULL
        AND  @LoopCounter <= @MaxId)
BEGIN
	PRINT CONCAT('Processing row: ', @LoopCounter)
   SET	@OldID = (SELECT TOP 1 MilestoneId FROM [dbo].[PortalWorkflow_MilestonesLibrary_MP]  WHERE LoopId = @LoopCounter)

	IF(@OldID != -1)	
	BEGIN
	INSERT INTO [dbo].[PortalWorkflow_MilestonesLibrary] OUTPUT Inserted.MilestoneId INTO @IdentityValue
		SELECT CONCAT(MilestoneName,'-B') AS MilestoneName,
Description,
Category,
Role,
Assignee,
...
   		FROM [dbo].[PortalWorkflow_MilestonesLibrary_MP]  WHERE LoopId = @LoopCounter
	...
	INSERT INTO [dbo].[PortalWorkflow_MilestonesLibrary_ID_Mapping]
	VALUES(@OldID, @NewID)
